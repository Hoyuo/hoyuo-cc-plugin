...
11. 기존 wiki 문서 전체에서 해당 용어가 언급되는 곳을 Grep 도구로 검색 (`wiki/concepts/`, `wiki/topics/` 대상)
12. 매칭된 문서에 `[[새 문서]]` wikilink를 첫 번째 언급 위치에 추가
13. 새 문서의 "관련 문서" 섹션에도 역방향 링크 추가

### Phase 5 — 메타 문서 갱신

14. `wiki/log.md` 에 작업 기록을 append한다
15. `wiki/overview.md` 가 존재하면 새 소스 반영하여 업데이트한다
...
모든 작업 완료 후 `wiki/log.md` 에 다음 형식으로 append:
